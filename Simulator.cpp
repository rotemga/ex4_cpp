...
Simulator::Simulator(vector <House*> houses1, vector <AbstractAlgorithm*> algos1, map<string, int> config1, vector<string> algoNames, vector<string> houseNames) :
houses(houses1), algorithms(algos1), config(config1), num_of_algos(algos1.size()), algosNames(algoNames),housesNames(houseNames) {
	for (size_t i = 0; i < num_of_algos*houses.size(); i++){
		robots.push_back(nullptr);
	}
